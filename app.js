...
// Routes
app.get('/', (req, res) => {
    res.render('index', { user: req.session.user, messages: req.flash('success')}); //login route
});
...
app.post('/login', (req, res) => {
  ...
  db.query(sql, [email, password], (err, results) => {
    ...
    if (results.length > 0) {
      // Successful login
      req.session.user = results[0]; // store user in session //login route
